...
for S in $(cat new_freqs.temp | awk '{print $2}'); #Corta la columna de taxIDs
do
    echo $S;
    grep -P "\t$S\t" genomes_names_ids.txt >> found.temp && echo $S >> yess.txt || echo $S >> noss.txt
done
...
# Prepara todo para la busqueda por clados. Obtienes tax_clades.tab

#ipython3 taxonome.py taxonomies.txt

# Nueva busqueda dada la lista A, busca coincidencias en la lista B e imprime las mismas.
# Var: FRECUENCIAS

#for S in $(cat new_freqs.csv | awk '{print $3}');
#do
#    grep $S tax_clades.tab >> cladooos.temp
#done
#
#echo "
#ya we
