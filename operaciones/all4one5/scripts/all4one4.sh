...
cut -f1,3,4,7,16,17,18,19,20,21,23- tabbed.tab > dom_fields.temp;
mv tabbed.tab raw_fields.tab;
mv raw_fields.tab control_lists/;

# Cortamos campos especificos y descripciones.
cut -f 11- dom_fields.temp | sed 's/\t/ /g' | sed 's/,/ /g' > description.temp;
cut -f -10 dom_fields.temp > data.temp;
